...
def find_folder(name: str, parent_id: str = None) -> tuple:
    """Cari folder berdasarkan nama. Return (id, link) atau (None, None)."""
...
def list_files_in_folder(folder_id: str) -> set:
    """Return set of filenames yang sudah ada di folder."""
...
# ─────────────────────────────────────────────
# 🤖  SYSTEM PROMPT
# Optimized: ~230 token (turun dari ~420), tetap human-readable.
# Special cases: green cast, low-light, backlight.
# ─────────────────────────────────────────────
...
def analyze_image(image_path: str) -> dict:
    """Kirim gambar 768px ke Gemma 4, return parameter koreksi.
    768px: cukup besar untuk detect subtle color spill, masih efisien token.
    """
...
# FIX #14: Ganti linear S-curve ke power curve via LUT.
# Lebih akurat perceptually, tidak butuh gamma decode.
# Pivot di midtone 128 (0.5), simetris di kedua sisi.
# ─────────────────────────────────────────────
...
def build_contrast_lut(contrast: float) -> np.ndarray:
    ...
    Pivot di 0.5 (midtone 128), simetris.
    """
...
# Dipakai untuk memutuskan apakah Gray World WB aman dipakai.
# ─────────────────────────────────────────────

def is_backlight(img: np.ndarray) -> bool:
    """
    Deteksi backlight/contre-jour:
    40% atas frame (langit/background) jauh lebih terang
    dari 60% bawah (subjek foreground), gap > 50/255.

    Gray World tidak valid untuk backlight karena background
    terang mendominasi mean channel dan hasilnya salah arah.
    """
...
# Pre-pass sebelum parameter fine-tuning Gemma.
# Efektif untuk dominant cast (green, cyan, orange, dll).
# TIDAK dipakai untuk foto backlight (lihat is_backlight).
# ─────────────────────────────────────────────

def gray_world_wb(img: np.ndarray, strength: float = 0.7) -> np.ndarray:
    """
    Gray World auto white balance.
    Asumsi: rata-rata scene harus abu-abu netral.
    strength < 1.0 untuk partial correction (lebih natural).
    """
    avg_b = np.mean(img[:, :, 0])
    avg_g = np.mean(img[:, :, 1])
    avg_r = np.mean(img[:, :, 2])
...
# ─────────────────────────────────────────────
# 🖼️  QUALITY GUARD
# Bandingkan green excess sebelum vs sesudah edit.
# Jika edit memperburuk cast, pakai original.
# ─────────────────────────────────────────────

def green_excess(img: np.ndarray) -> float:
    """Hitung seberapa dominan green channel dibanding rata-rata R dan B."""
...
#
# Pipeline (research-based, semua di gamma-encoded space):
#   Auto WB (Gray World) → WB fine-tune (warmth/tint) →
#   Brightness → Contrast LUT → Highlights/Shadows →
#   Blacks/Whites → Saturation → Vibrance →
#   Clarity → Sharpness → simpan
#
# FIX #14 — Hapus gamma decode/encode (penyebab foto pucat).
#            Ganti contrast ke power curve LUT di gamma space.
# FIX #13 — Deteksi backlight, skip Gray World WB.
# FIX #4  — Gray World auto WB pre-pass.
# FIX #1  — WB multiplicative, bukan additive.
# FIX #6  — norm di-update setelah brightness & contrast.
# FIX #7  — norm di-update sebelum blacks & whites.
# FIX #5,9— Vibrance multiplicative, mask dari s original.
# FIX #8  — Clarity sigma proporsional ke resolusi.
# FIX #12 — Sleep di path skip.
# ─────────────────────────────────────────────
...
def apply_corrections(input_path: str, params: dict, output_path: str) -> str:
    ...
    # ── FIX #13 + #4: Gray World WB — skip jika backlight ───────────────────
    # Backlight = distribusi pixel sangat tidak merata → Gray World salah arah.
    if not is_backlight(img):
        img = gray_world_wb(img, strength=0.7)

    # 1. WHITE BALANCE — Warmth & Tint (FIX #1: multiplicative)
    # Multiplicative (×factor) bekerja proporsional di semua brightness level,
    # lebih efektif dari additive (+value) untuk netralisasi cast yang kuat.
    ...
    wb_g = 1.0 - tint   / 200.0   # tint +15  → G ×0.925 (magenta push = kurangi green) ✓
    ...
    # 3. CONTRAST — Power curve LUT (FIX #14)
    # Tidak pakai linear (x-128)*c+128 karena tidak perceptually akurat.
    # Power curve pivot di midtone 0.5, simetris, natural untuk mata.
    # Tidak butuh gamma decode — justru bekerja lebih baik di gamma space.
    lut = build_contrast_lut(params.get("contrast", 1.0))
    img = cv2.LUT(img.astype(np.uint8), lut).astype(np.float32)

    # ── FIX #6: Hitung norm SETELAH brightness & contrast ───────────────────
    ...
    # ── FIX #7: Update norm SEBELUM blacks & whites ────────────────────────
    norm = np.clip(img, 0, 255) / 255.0
    ...
    # ── FIX #5: Simpan s original SEBELUM saturation ────────────────────────
    s_original = hsv[:, :, 1].copy()
    s = np.clip(s_original * params.get("saturation", 1.0), 0, 255)

    # ── FIX #9: Vibrance multiplicative, mask dari s original ────────────────
    # Area low-saturation mendapat boost lebih kuat (skin-safe).
    # Mask dari s_original agar tidak double-count efek saturation.
    ...
    # ── FIX #8: Clarity sigma proporsional ke resolusi ──────────────────────
    # ~0.4% dari dimensi terpanjang: 4000px→16, 1920px→7.7, 512px→2.0
    ...
    # ── FIX #8: Sharpness sigma proporsional ke resolusi ────────────────────
    sigma_sharp = max(h_px, w_px) * 0.0006  # ~0.06%: 4000px→2.4, 1920px→1.2, 512px→0.3
    ...
    # ── Quality Guard: cek apakah edit memperburuk green cast ───────────────
...
# Resume: cek ap folder target sudah ada
...
# List file yang sudah diproses di target
...
for f in tqdm(images, desc="Memproses"):
    ...
    # Skip jika file sudah ada di target
    if fname in existing_files:
        skip += 1
        print(f"  ⏭️ {fname}: sudah ada, skip")
        log.append({"file": fname, "status": "skip"})
        time.sleep(0.5)  # FIX #12: minimal sleep di skip, cegah Drive API rate limit
        continue
